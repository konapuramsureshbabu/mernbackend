...
exports.userpost=async(req,res)=>{
    ...
    try{
        ...
        if(preuser){
            res.status(401).json('This users is already exist in our database');
        }else{
...
exports.userget=async(req,res)=>{
    ...
    try{
        const skip =(page-1)*ITEM_PER_PAGE // 0*4=0
        const count =await users.countDocuments(query);
        const usersdata=await users.find(query)
        .sort({datecreated:sort=="new" ? -1:1}).limit(ITEM_PER_PAGE).skip(skip)
        const pageCount=Math.ceil(count/ITEM_PER_PAGE); //8/4=2 pages 
